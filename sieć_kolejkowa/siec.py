...
class Stage:
    def __init__(self, name: str, capacity: int, queue_limit: Optional[int] = None):
        self.name = name
        self.capacity = capacity  # Maksymalna liczba zgłoszeń przetwarzanych w jednostce czasu
        self.queue_limit = queue_limit  # Maksymalna liczba zgłoszeń w kolejce (opcjonalnie)
        self.queue: List[Request] = []
        self.transit_queue: List[Request] = []  # Zgłoszenia w tranzycie do kolejnego etapu
    ...
    def process_request(self, request: Request, current_time: int):
        ...
        request.next_stage_arrival_time = current_time + 1  # Przybędzie za 1 jednostkę czasu
...
class LiniaProdukcyjna(Stage):
    def __init__(self):
        super().__init__('Linia Produkcyjna', capacity=0)  # Początkowa wartość nie ma znaczenia
...
def symulacja(czas_trwania: int):
    ...
    num_blue_shipped = len([r for r in wysylka.processed_requests if r.type == 'niebieski'])  # Powinno być zero
    ...
    for stage in wszystkie_etapy:
        ...
        print(f"  Średnia długość kolejki: {sum(stage.queue_lengths)/len(stage.queue_lengths):.2f}")
        ...
        print(f"  Całkowity czas oczekiwania w kolejce: {stage.total_waiting_time} jednostek czasu")
        print("")
